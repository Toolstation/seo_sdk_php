 ...
 * Bazaarvoice Class
 *
 * When you instantiate the Bazaarvoice class, pass it's constructor an array
...
class Bazaarvoice
{
    ...
    public function __construct($params = array())
    {
        ...
        if (isset($params['page_url'])) {
            $this->config['bv_page_data'] = Utility::parseUrlParameters($params['page_url']);
        }

        // Extract bvstate if present and parse that into a set of useful values.
        if (isset($this->config['bv_page_data']['bvstate'])) {
            $this->config['page_params'] = Utility::getBVStateParams($this->config['bv_page_data']['bvstate']);
        }
        ...
        $this->config['base_url'] = Utility::removeUrlParam($this->config['base_url'], 'bvstate');
        $this->config['base_url'] = Utility::removeUrlParam($this->config['base_url'], 'bvrrp');
        $this->config['base_url'] = Utility::removeUrlParam($this->config['base_url'], 'bvqap');
        $this->config['base_url'] = Utility::removeUrlParam($this->config['base_url'], 'bvsyp');
        ...
        $ct = isset($this->config['page_params']['content_type'])
            ? $this->config['page_params']['content_type']
            ...
            switch ($ct) {
                case 'reviews': {
                    $st = isset($this->config['page_params']['subject_type'])
                        ? $this->config['page_params']['subject_type']
    ...
    protected function validateParameters($params) {
        if (!is_array($params)) {
            throw new \Exception(
                'Bazaarvoice class constructor argument $params must be an array.'
            );
        }
        ...
        if (empty($params['bv_root_folder'])) {
            throw new \Exception(
                'Bazaarvoice class constructor argument $params is missing required bv_root_folder key. An ' .
        ...
        if (empty($params['subject_id'])) {
            throw new \Exception(
                'Bazaarvoice class constructor argument $params is missing required subject_id key. An ' .
